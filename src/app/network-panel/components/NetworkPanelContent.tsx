...
const QUICK_COMMANDS = [
  ...
  { label: 'Groups', cmd: 'POST /groups {"limit":50,"offset":0,"raw":true}' },
  { label: 'Auth Ping', cmd: 'POST /info {}' },
  { label: 'Captcha Balance', cmd: 'POST /captcha/balance {}' },
  { label: 'CAPTCHA Status', cmd: 'POST /cmd {"command":"captcha-status"}' },
  { label: 'Autha Stats', cmd: 'GET /autha/stats' },
];

const MESSAGES_KEY = 'toque_network_messages';
const LATENCY_KEY = 'toque_network_latency';
...
const INITIAL_MESSAGES: ChatMessage[] = [
  {
    id: 'm0',
    role: 'system',
    content: '# Toque API Terminal — type a command like "GET /health" or "POST /info {}"',
    timestamp: '--:--:--',
  },
];
...
export default function NetworkPanelContent() {
  const [messages, setMessages] = useState<ChatMessage[]>(() =>
    readStored<ChatMessage[]>(MESSAGES_KEY, INITIAL_MESSAGES)
  );
  const [input, setInput] = useState('');
  const [isSending, setIsSending] = useState(false);
  const [wsStatus, setWsStatus] = useState<'connected' | 'disconnected'>('disconnected');
  const [latencyHistory, setLatencyHistory] = useState<number[]>(() =>
    readStored<number[]>(LATENCY_KEY, [])
  );
  ...
  useEffect(() => {
    localStorage.setItem(LATENCY_KEY, JSON.stringify(latencyHistory));
  }, [latencyHistory]);
  ...
  const handleSend = async (rawInput?: string) => {
    ...
    setWsStatus(result.ok ? 'connected' : 'disconnected');
